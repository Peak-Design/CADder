...
work with, a Refresh Model that keeps your rig, and a warning when CADder
and CADder Bridge do not match. Blender 5.1 or later. Install the zip for
your platform the same way as the first time. This release replaces 1.1.0.

## New

- **CADder and CADder Bridge say when their versions do not match.** The
  two work together when the first two numbers of their versions are the
  same: CADder 1.1.1 works with CADder Bridge 1.1.0. When they do not
  match, the **CADder** tab says so as soon as SolidWorks runs, before you
  send anything, with a button that opens the download page of the one to
  update. See
...
Install [CADder Bridge 1.1.1](https://github.com/Peak-Design/CADder-SW-Bridge/releases/latest)
in SolidWorks with this release. It has the SolidWorks half of the rig
engine improvements, and it asks before it sends to a CADder that does not
match. CADder 1.1 works with CADder Bridge 1.1. The new rule for when
Refresh Model is offered needs both of them at 1.1.
