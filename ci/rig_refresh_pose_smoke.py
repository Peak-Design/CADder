...
def export(tmp, parts, clip_x):
    man = os.path.join(tmp, "app.rig.json")
    with open(man, "w", encoding="utf-8") as fh:
        json.dump(manifest(parts, clip_x), fh)
    return write_mesh(os.path.join(tmp, "app.swmesh"), parts), man
